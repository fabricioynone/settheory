...
\begin{proof}
    ...
    Se $t$ é uma variável, digamos, $x_i$, então a conclusão segue da tautologia $A(x_i)\to A(x_i)$, seguida das introduções dos quantificadores restritos.

    Se $t$ é uma constante $c$, então $T^A\vdash A(c^A)$ pela definição de interpretação.
    Logo, por introduções de implicações e quantificadores restritos, obtemos
    \[
        T^A\vdash \forall x_1\in A\cdots\forall x_n\in A\,A(c^A).
    \]

    Suponha agora que $t$ é da forma $f(t_1,\dots,t_m)$.
    Pela hipótese indutiva, para cada $1\leq i\leq m$,
    \[
    ...
    \]
    Instanciando essa fórmula com os termos $t_1^A,\dots,t_m^A$, obtemos
    \[
        T^A\vdash
        A(t_1^A)\wedge\cdots\wedge A(t_m^A)\to
        A(f^A(t_1^A,\dots,t_m^A)).
    \]
    Pelas regras proposicionais derivadas para $\wedge$ e $\to$, concluímos
    \[
        T^A\vdash \forall x_1\in A\cdots\forall x_n\in A\,
        A(f^A(t_1^A,\dots,t_m^A)).
    \]
    Como $t^A$ é $f^A(t_1^A,\dots,t_m^A)$, isso prova a tese.
\end{proof}
...
\begin{metalemma}[Axiomas lógicos relativizados]\label{mlem:axiomasLogicosInterpretacao}
    ...
    \[
        T^A\vdash \forall x_1\in A\cdots \forall x_n\in A\,\alpha^A.
    \]
\end{metalemma}

\begin{proof}
    Seja $\alpha$ um axioma lógico da linguagem de $S$, e sejam $x_1,\dots,x_n$ exatamente as variáveis livres de $\alpha$.
    Provaremos que
    \[
        T^A\vdash \forall x_1\in A\cdots \forall x_n\in A\,\alpha^A.
    \]
    ...
    Se $\alpha$ é uma instância de um desses esquemas, então $\alpha^A$ é uma instância do mesmo esquema, agora na linguagem de $T^A$.
    Logo, $T^A\vdash \alpha^A$.
    Aplicando repetidamente introdução de implicação e generalização universal, obtemos
    \[
        T^A\vdash \forall x_1\in A\cdots\forall x_n\in A\,\alpha^A.
    \]

    O caso FO16 também é imediato.
    Se $\alpha$ é $x=x$, então $\alpha^A$ é $x=x$, que é uma instância de FO16 na linguagem de $T^A$.
    Portanto, como acima,
    \[
        T^A\vdash \forall x\in A\, (x=x).
    \]
