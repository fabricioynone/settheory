...
\begin{metalemma}\label{mlem:substituicaoEInterpretacao}
    Seja $T$ uma teoria de primeira ordem e seja $T^A$ uma interpretação de um léxico $\mathcal L'$ em $T$.
...
\begin{proof}
    ...
    Assim, qualquer generalização de $\alpha^A$ é um teorema de $T^A$.

    O caso FO16 também é imediato.
    Se $\alpha$ é $x=x$, então $\alpha^A$ é $x=x$, que é um axioma lógico da linguagem de $T^A$.
    Assim, qualquer generalização de $\alpha^A$ é um teorema de $T^A$.
    ...
    Tal fórmula é um axioma da linguagem de $T^A$, e, portanto, qualquer generalização de $\alpha^A$ é um teorema de $T^A$.

    O caso FO18 é análogo.

    Suponha agora que $\alpha$ é uma instância de FO14, isto é, $\alpha$ é da forma $\forall z\,\varphi\to \varphi[z/t]$,
    em que $z$ é substituível por $t$ em $\varphi$.
    Pelo Metalema~\ref{mlem:substituicaoEInterpretacao}, a substituição de $z$ por $t^A$ em $\varphi^A$ é válida e $(\varphi[z/t])^A$ é $\varphi^A[z/t^A]$.

    Assim, $\alpha^A$ é $\forall z(A(z)\to\varphi^A)\to \varphi^A[z/t^A]$.

    Como $A(z)$ é uma fórmula atômica, $z$ é substituível por $t^A$ em $A(z)$, e temos que, pelo axioma FO14 em $T^A$,
    \[
        T^A\vdash
    ...
    \[
        T^A\vdash\forall x_1\in A\cdots \forall x_n\in A\,
        \forall z(A(z)\to\varphi^A)
        \to
        \bigl(A(t^A)\to\varphi^A[z/t^A]\bigr).
    \]

     Se todas as variáveis que ocorrem em $t$ estão entre $x_1,\dots,x_n$, então, pelo metalema sobre termos interpretados pertencentes a $A$,
    \[
        T^A\vdash
        \forall x_1\in A\cdots \forall x_n\in A\, A(t^A).
    \]
    Combinando isso com a instância de FO14 acima, obtemos
    \[
        T^A\vdash
        \forall x_1\in A\cdots \forall x_n\in A\,
        \bigl(\forall z(A(z)\to\varphi^A)\to \varphi^A[z/t^A]\bigr),
    \]
    isto é,
    \[
        T^A\vdash
    ...
    Resta considerar o caso em que alguma variável de $t$ não está entre $x_1,\dots,x_n$.
    Nesse caso, $z$ não ocorre livre em $\varphi$.
    De fato, se $z$ ocorresse livre em $\varphi$, então, como a substituição de $z$ por $t$ em $\varphi$ é válida, toda variável de $t$ ocorreria livre em $\varphi[z/t]$, e portanto ocorreria livre em $\alpha$.

    Assim, $\varphi[z/t]$ é a própria $\varphi$, e $\alpha^A$ é
    \[
        \forall z(A(z)\to\varphi^A)\to\varphi^A.
    \]
    Como $T^A\vdash \exists w\,A(w)$, tomando $w$ que não ocorre livre em $\varphi^A$, obtemos, por FO14,
    \[
        T^A\vdash
    ...
    \]
    Logo, como $T^A\vdash \exists w\,A(w)$, temos $T^A\vdash \alpha^A$.
    Portanto, também nesse caso,
    \[
        T^A\vdash
        \forall x_1\in A\cdots \forall x_n\in A\,\alpha^A.
    \]

    O axioma FO15 é tratado de forma análoga.
\end{proof}